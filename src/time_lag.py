...
def time_lag_between_groups(time_lag, marker_list, group1, group2, 
    ...
    # # Use only the upper triangular part of the matrix to avoid redundancy
    # results = []
    # for i in range(num_markers):
    #     for j in range(i+1, num_markers):  # Only use upper triangular part
    #         key = f"{marker_list[i]}_to_{marker_list[j]}"
    #         group1_values[key] = []
    #         group2_values[key] = []
            
    #         # Collect values for group1
    #         for subject in subjects_group1:
    #             try:
    #                 value = time_lag[group1][subject][task][kinematics][direction]['avg'][i, j]
    #                 group1_values[key].append(value)
    #             except (TypeError, KeyError):
    #                 pass  # Skip if data is missing
            
    #         # Collect values for group2
    #         for subject in subjects_group2:
    #             try:
    #                 value = time_lag[group2][subject][task][kinematics][direction]['avg'][i, j]
    #                 group2_values[key].append(value)
    #             except (TypeError, KeyError):
    #                 pass  # Skip if data is missing
            
    #         # Perform statistical test if we have data for both groups
    #         if group1_values[key] and group2_values[key]:
    #             # Independent t-test
    #             t_stat, p_val = stats.ttest_ind(group1_values[key], group2_values[key], equal_var=False)
                
    #             # Calculate Cohen's d effect size
    #             mean1 = np.mean(group1_values[key])
    #             mean2 = np.mean(group2_values[key])
    #             pooled_std = np.sqrt((np.std(group1_values[key])**2 + np.std(group2_values[key])**2) / 2)
                
    #             effect = (mean1 - mean2) / pooled_std if pooled_std > 0 else 0
    #             diff = mean1 - mean2
                
    #             results.append({
    #                 'marker_pair': key,
    #                 'p_value': np.round(p_val, 4),
    #                 'cohens_d': np.round(effect, 2),
    #                 'mean_diff': np.round(diff, 2),
    #                 f'mean_{group1}': np.round(mean1, 2),
    #                 f'mean_{group2}': np.round(mean2, 2)
    #             })
    ...
    #     if apply_correction:
    #         from statsmodels.stats.multitest import multipletests
    #         reject, corrected_p, _, _ = multipletests(results_df['p_value'].values, method='fdr_bh')
    #         results_df['corrected_p_value'] = corrected_p
    #         results_df['significant'] = reject
    #         sig_threshold = 'corrected_p_value'
    #     else:
    #         # Without correction, just use raw p-values
    #         results_df['significant'] = results_df['p_value'] < 0.05
    #         sig_threshold = 'p_value'
        
    #     # Create a separate DataFrame with only significant results
    #     sig_results = results_df[results_df['significant']].copy()
        
    #     # Organize results by significance level
    #     significance_dict = {
    #         'high_significance': results_df[results_df[sig_threshold] < 0.01].copy(),
    #         'medium_significance': results_df[(results_df[sig_threshold] >= 0.01) & 
    #                                         (results_df[sig_threshold] < 0.05)].copy(),
    #         'no_significance': results_df[results_df[sig_threshold] >= 0.05].copy()
    #     }
    # else:
    #     sig_results = pd.DataFrame()
    #     significance_dict = {
    #         'high_significance': pd.DataFrame(),
    #         'medium_significance': pd.DataFrame(),
    #         'no_significance': pd.DataFrame()
    #     }
    
    # return results_df, sig_results, significance_dict
