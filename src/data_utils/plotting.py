...
def plot_avg_matrices(avg_group1, avg_group2, group1, group2, marker_list, task, direction, matrix_type, result_base_path, rho, p_value, suptitle, figname):
    ...
    print(f"  [plot_avg_matrices] {matrix_type}/{direction}/{task}: min={global_min:.3f}, max={global_max:.3f}, has_neg={(global_min < 0)}")
    ...
    result_folder = Path(result_base_path) / "kinectome_characteristics"
...
def plot_correlation_heatmap(corr_matrix, marker_list, title='Maximum Cross-Correlation', result_base_path='results'):
...
def plot_lag_heatmap(lag_matrix, markers_list, title='Time Lag at Maximum Correlation', result_base_path='results'):
...
def draw_graph_with_weights(G, result_base_path='results'):
...
def draw_graph_with_selected_weights(G, selected_edges=None, result_base_path='results'):
    """
...
def visualise_kinectome(kinectome_data, figname, marker_list, sub_id, task_name, kinematics, result_base_path, what_to_plot):
    ...
    # Create the heatmaps — lower triangle only (matrix is symmetric)
    titles = ['Anteroposterior direction', 'Mediolateral direction', 'Vertical direction']
    for i, matrix in enumerate(kinectome_matrices):
        ax = axes[i]

        # Mask the upper triangle (keep diagonal and below)
        mask = np.triu(np.ones_like(matrix, dtype=bool), k=1)

        sns.heatmap(matrix, mask=mask, cmap="coolwarm", vmin=scales[i][0], vmax=scales[i][1],
...
def plot_cc(DATA_PATH,sub_id,task_name,tracksys,run,kinematics,MARKER_LIST,threshold_list=[0.2,0.4,0.6,0.8]):
    ...
    from src.graph_utils.graphs import build_graph, clustering_coef
...
def event_plot_components(DATA_PATH,sub_id,task_name,tracksys,run,kinematics,MARKER_LIST,threshold_list=[0.2,0.4,0.6,0.8],direction=1):
    from src.data_utils import data_loader
    from src.graph_utils.graphs import build_graph, clustering_coef, cc_connected_components
...
def plot_permutation_histogram(rhos, true_rho, perm_p, results_path, task, kinematic, direction, matrix_type, correlation_method):
    ...
    if matrix_type == 'allegiance' or matrix_type == 'allegiance_std':
        save_dir = Path(results_path) / "modularity" / "allegiance_matrices"
    else:
        save_dir = Path(results_path) / "kinectome_characteristics"
    save_dir.mkdir(parents=True, exist_ok=True)
    plt.savefig(save_dir / f'permutation_{task}_{kinematic}_{direction}_{matrix_type}_{correlation_method}.png', dpi=600)
...
def create_bootstrap_plots(bootstrap_results, observed_rhos, task_names, kinematics_list, matrix_type, result_base_path="results"):
    ...
    for kinematic in kinematics_list:
        for task in task_names:
            ...
            for row, mt in enumerate(matrix_types):
                # bootstrap_results is keyed by matrix_type at top level
                bt = bootstrap_results.get(mt, bootstrap_results)  # handle both structures
                obs = observed_rhos.get(mt, observed_rhos)

                for col, direction in enumerate(directions):
                    ax = axes[row, col]

                    try:
                        bootstrap_rhos = bt[task][kinematic][direction]
                        observed_rho   = obs[task][kinematic][direction]
                    except (KeyError, TypeError):
                        bootstrap_rhos = []
                        observed_rho   = None

                    if bootstrap_rhos and observed_rho is not None:
                        ax.hist(bootstrap_rhos, bins=30, alpha=0.7,
                                color=row_colors[mt], edgecolor='black')
                        ax.axvline(observed_rho, color='red', linestyle='--',
                                   linewidth=2, label=f'Observed ρ = {observed_rho:.3f}')
                        bootstrap_mean = np.mean(bootstrap_rhos)
                        ax.axvline(bootstrap_mean, color='orange', linestyle=':',
                                   linewidth=2, label=f'Bootstrap μ = {bootstrap_mean:.3f}')
                        ax.legend(fontsize=8)
                    else:
                        ax.text(0.5, 0.5, 'No data', ha='center', va='center',
                                transform=ax.transAxes, fontsize=10, color='grey')

                    ax.set_title(f'{row_labels[mt]} — {direction}', fontsize=10)
                    ax.set_xlabel('Correlation (ρ)', fontsize=9)
                    ax.set_ylabel('Frequency', fontsize=9)
                    ax.grid(True, alpha=0.3)

            plt.tight_layout()
            filename = f'bootstrap_{task}_{kinematic}.png'
...
def plot_community_nodal_strength_single_task(df, consensus_communities, task_prefix,
                                              save_dir="./plots", group_order=None):
    """
    Nodal-strength plots for a task WITHOUT speed conditions (e.g. walkStroop).

    Unlike plot_community_nodal_strength (which draws one box per speed and
    tests within-group across speeds), this draws a single box per group for
    each segment and tests only the between-group difference (Mann-Whitney U,
    FDR-corrected across segments within each community x direction).

    Parameters
    ----------
    df : pandas.DataFrame
        Centrality DataFrame with columns '{segment}_{task_prefix}_{direction}'.
    consensus_communities : list of (set or list)
        Communities of body segments; subplot order follows list order.
    task_prefix : str
        Column task token to plot (e.g. 'stroop'), matching export_centrality_to_csv.
    save_dir : str
        Output directory.
    group_order : list or None
        Explicit group order; defaults to the order found in df['group'].
    """
    import os
    import numpy as np
    import pandas as pd
    ...
    for community_idx, community in enumerate(consensus_communities):
        ...
        for direction in directions:
            ...
            for segment in community_segments:
                col = f"{segment}_{task_prefix}_{direction}"
                if col not in df.columns:
                    continue
                vals_by_group = {}
                for group in groups:
                    v = df.loc[df['group'] == group, col].dropna().to_numpy()
                    vals_by_group[group] = v
                seg_group_values[segment] = vals_by_group

                if len(groups) == 2:
                    g1, g2 = groups
                    if len(vals_by_group[g1]) >= 2 and len(vals_by_group[g2]) >= 2:
                        try:
                            _, p = stats.mannwhitneyu(vals_by_group[g1], vals_by_group[g2],
                                                      alternative='two-sided')
                        except ValueError:
                            p = np.nan
                        seg_pvals[segment] = p

            # FDR correction across segments that yielded a p-value
            corrected = {}
            if seg_pvals:
                seg_keys = list(seg_pvals.keys())
                raw = [seg_pvals[k] for k in seg_keys]
                mask = [not (r is None or np.isnan(r)) for r in raw]
                if any(mask):
                    raw_valid = [raw[i] for i in range(len(raw)) if mask[i]]
                    _, p_corr, _, _ = multipletests(raw_valid, alpha=0.05, method='fdr_bh')
                    j = 0
                    for i, k in enumerate(seg_keys):
                        if mask[i]:
                            corrected[k] = p_corr[j]
                            j += 1
            ...
            for seg_idx, segment in enumerate(community_segments):
                ...
                if col not in df.columns or segment not in seg_group_values:
                    ax.set_visible(False)
                    continue

                vals_by_group = seg_group_values[segment]
                box_data, box_positions, box_colors = [], [], []
                pos = 0
                group_centers = {}
                for group in groups:
                    v = vals_by_group[group]
                    if len(v) > 0:
                        box_data.append(v)
                        box_positions.append(pos)
                        box_colors.append(group_colors.get(group, 'gray'))
                ...
                if box_data:
                    ...
                    # Between-group significance bracket
                    if segment in corrected and len(box_positions) >= 2:
                        symbol = sig_symbol(corrected[segment])
                        if symbol:
                            y = max(np.max(d) for d in box_data)
                            h = y + 0.05 * (abs(y) + 1)
                            x1, x2 = box_positions[0], box_positions[1]
                            ax.plot([x1, x1, x2, x2],
                                    [y, h, h, y], 'k-', linewidth=1)
                            ax.text((x1 + x2) / 2, h, symbol,
                                    ha='center', va='bottom', fontsize=16, fontweight='bold')

                    ax.set_xticks(list(group_centers.values()))
                    ax.set_xticklabels(list(group_centers.keys()), fontsize=11)

                ax.set_title(f'{segment}', fontsize=14, fontweight='bold')
                if seg_idx % n_cols == 0:
                    ax.set_ylabel('Nodal Strength')
            ...
            fig.suptitle(f'Community {community_idx + 1} — {task_prefix} — {direction}',
                         fontsize=15, fontweight='bold')
            plt.tight_layout()
            filename = f"community_{community_idx + 1}_{task_prefix}_{direction}_nodal_strength.png"
            filepath = os.path.join(save_dir, filename)
            plt.savefig(filepath, dpi=300, bbox_inches='tight')
            plt.close(fig)
            print(f"Saved plot: {filepath}")