...
def load_kinectomes(base_path, sub_id, task_name, tracksys, run, kinematics, full, correlation_method, interpol=False):
    ...
    try:
        ...
        if run:            
            if full:
                if interpol:
                    relevant_files = [file for file in file_list if all(x in file for x in [task_name, tracksys, run, kinematics, correlation_method, 'full', 'interpol'])]
                else:
                    relevant_files = [file for file in file_list if all(x in file for x in [task_name, tracksys, run, kinematics, correlation_method, 'full']) and 'interpol' not in file]
            else:
                if interpol:
                    relevant_files = [file for file in file_list if all(x in file for x in [task_name, tracksys, run, kinematics, correlation_method, 'interpol']) and 'full' not in file]
                else:
                    relevant_files = [file for file in file_list if all(x in file for x in [task_name, tracksys, run, kinematics, correlation_method]) and 'full' not in file and 'interpol' not in file]
        else:            
            if full:
                if interpol:
                    relevant_files = [file for file in file_list if all(x in file for x in [task_name, tracksys, kinematics, correlation_method, 'full', 'interpol'])]
                else:
                    relevant_files = [file for file in file_list if all(x in file for x in [task_name, tracksys, kinematics, correlation_method, 'full']) and 'interpol' not in file]
            else:
                if interpol:
                    relevant_files = [file for file in file_list if all(x in file for x in [task_name, tracksys, kinematics, correlation_method, 'interpol']) and 'full' not in file]
                else:
                    relevant_files = [file for file in file_list if all(x in file for x in [task_name, tracksys, kinematics, correlation_method]) and 'full' not in file and 'interpol' not in file]
