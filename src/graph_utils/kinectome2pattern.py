...
def strongest_pattern_subgraph(G, length: int, start_node):
    """
    Returns a path subgraph of G of the specified length, starting from a given node,
    walking through the graph via edges with the highest absolte weight and without repeating nodes.
    ...
    while len(visited) < length:
        ...
        next_node = max(neighbors, key=lambda x: abs(x[1]))[0]
        visited.append(next_node)
        current_node = next_node
